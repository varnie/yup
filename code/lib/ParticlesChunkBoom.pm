...
#TODO:
sub init {
    my ($self, $center_x, $center_y, $pos) = @_;
...
use Inline C => <<'END';
void draw_c(SV *self, SDL_Surface *dst, int map_offset_x, int map_offset_y) {

    HV *obj_hash = MUTABLE_HV(SvRV(self));

    AV *av_items = (AV *) SvRV(*hv_fetch(obj_hash, "items", 5, 0)); //5 is strlen("items")
    const int particles_count = av_top_index(av_items);

    const int size = SvIV(*hv_fetch(obj_hash, "size", 4, 0)); //4 is strlen("size")

    SV *img_sv = *hv_fetch(obj_hash, "img", 3, 0); //3 is strlen("img")
    void **pointers = (void**)(SvIV((SV*)SvRV(img_sv)));
    SDL_Surface *src = (SDL_Surface*)(pointers[0]);
    ...
    for (i = 0; i < particles_count; ++i) {
        SV *sv_particle = *av_fetch(av_items, i, 0);
        HV *particle_obj_hash = MUTABLE_HV(SvRV(sv_particle));

        if (SvIV(*hv_fetch(particle_obj_hash, "ttl", 3, 0)) > 0) { //3 is strlen("ttl")
            //draw the particle

            //find src_pos values
            AV *av_particle_src_pos = (AV *) SvRV(*hv_fetch(particle_obj_hash, "src_pos", 7, 0)); //7 is strlen("src_pos")
